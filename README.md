...
- **Build images** — Images → Build Image: pick a context folder and Dockerfile, set tag/build-args, optionally **forward your SSH agent** (for `RUN --mount=type=ssh` against private repos), and Davit drives the platform's BuildKit builder (starting it if needed), then loads and tags the result into the image store. `Davit build -t <tag> <context-dir>` headless.
